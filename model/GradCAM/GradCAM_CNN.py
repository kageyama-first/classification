...
def _project_cnn_module_dir() -> Path:
    return (_project_root() / "model" / "CNN" / "code").resolve()
...
def main():
    ...
    parser.add_argument("--ckpt", type=str, default="model/CNN/code/best_model.pth")
    parser.add_argument("--ckpt-simple", type=str, default="model/CNN/code/best_model.pth")
    parser.add_argument("--ckpt-advanced", type=str, default="model/GradCAM/best_model_advanced.pth")
    parser.add_argument("--ckpt-cbam", type=str, default="model/GradCAM/outputs/after_CBAM/cnn_cbam/best_model.pth")
    ...
    if str(args.mode) == "compare":
        simple_model, _simple_name = load_model(kind="simple", num_classes=len(class_names), ckpt_path=_resolve_ckpt(args.ckpt_simple))
        cbam_model, _cbam_name = load_model(kind="cbam", num_classes=len(class_names), ckpt_path=_resolve_ckpt(args.ckpt_cbam))
    ...
    if str(args.model) == "advanced":
        ckpt = _resolve_ckpt(args.ckpt_advanced)
