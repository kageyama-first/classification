...
mkdir -p model/CNN/code
mkdir -p model/GradCAM/outputs
mkdir -p model/GradCAM/outputs/after_CBAM/cnn_cbam
mkdir -p results
...
# 查找 CNN 模型权重（Grad-CAM 仅支持 SimpleCNN / AdvancedCNN / CBAM）
# 路径需相对项目根目录，供 GradCAM_CNN.py 的 _resolve_ckpt 解析
MODEL_KIND=""
CKPT_PATH=""
if [ -f "../CNN/code/best_model.pth" ]; then
    CKPT_PATH="model/CNN/code/best_model.pth"
    MODEL_KIND="simple"
elif [ -f "outputs/after_CBAM/cnn_cbam/best_model.pth" ]; then
    CKPT_PATH="model/GradCAM/outputs/after_CBAM/cnn_cbam/best_model.pth"
    MODEL_KIND="cbam"
elif [ -f "best_model_advanced.pth" ] && [ "$(wc -c < "best_model_advanced.pth" | tr -d ' ')" -gt 1024 ]; then
    CKPT_PATH="model/GradCAM/best_model_advanced.pth"
    MODEL_KIND="advanced"
else
    echo "❌ 错误: 未找到 CNN 模型权重！" | tee -a "../../$LOG_FILE"
    echo "请将权重放到以下任一位置后重试:" | tee -a "../../$LOG_FILE"
    echo "  - model/CNN/code/best_model.pth (SimpleCNN)" | tee -a "../../$LOG_FILE"
    echo "  - model/GradCAM/best_model_advanced.pth (AdvancedCNN)" | tee -a "../../$LOG_FILE"
    echo "  - model/GradCAM/outputs/after_CBAM/cnn_cbam/best_model.pth (CBAM)" | tee -a "../../$LOG_FILE"
    cd ../..
    exit 1
fi

echo "开始运行 Grad-CAM 实验..." | tee -a "../../$LOG_FILE"
echo "使用 CNN 模型: $MODEL_KIND | 权重: $CKPT_PATH" | tee -a "../../$LOG_FILE"
...
echo "  - CNN 模型权重: model/CNN/code/best_model.pth"
